...
__kernel void rgb2lmshsi(__global const float* rgb, __global float* result, __global const int* imageHeight, __global const int* channels, __global const float* white)
{
    float r, g, b, x, y, z, l,m,s;
    int i = get_global_id(0);
    int j = get_global_id(1);
    ...
    // LMS
    r = (0.7328f * x + 0.4296f * y - 0.1624f * z)/white[0];
    g = (-0.7036f * x + 1.6975f * y + 0.0061f * z)/white[1];
    b = (0.0030f * x + 0.0136f * y + 0.9834f * z)/white[2];
...
__kernel void lmshsi2rgb(__global const float* hsi, __global float* result, __global const int* imageHeight, __global const int* channels, __global float* white)
{
    int i = get_global_id(0);
    int j = get_global_id(1);
    int h = imageHeight[0];
	...
	l = value(hue + 0.0f, intensity, saturation)*white[0];
	m = value(hue + 4.0f, intensity, saturation)*white[1];
	s = value(hue + 2.0f, intensity, saturation)*white[2];
	}
...
__kernel void white_balance_half_point(__global const float* lms, __global float* result, __global const int* imageHeight, __global const int* channels, __global const float* factors, __global const float* white)
{
    float r, g, b, x, y, z;
    int i = get_global_id(0);
    int j = get_global_id(1);
	...
	result[idx] = lms[idx]/factors[0] * white[0] * 0.5f;
	result[idx+1] = lms[idx+1]/factors[1] * white[1] * 0.5f;
	result[idx+2] = lms[idx+2]/factors[2] * white[2] * 0.5f;
}
